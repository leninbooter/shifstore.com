...
<table id="total-tab" width="100%">
	...
	{if isset($order->payment_fee) && $order->payment_fee > 0}
		<tr>
			<td class="grey" width="70%">{l s='Administration fees' pdf='true'}</td>
			<td class="white" width="30%">
				{if $tax_excluded_display}- {displayPrice currency=$order->id_currency price=($order->payment_fee / (1+($order->payment_fee_rate/100)))}{else}- {displayPrice currency=$order->id_currency price=$order->payment_fee}{/if}
			</td>
		</tr>
	{/if}
