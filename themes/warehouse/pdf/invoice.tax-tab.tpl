...
{elseif (isset($tax_breakdowns) && $tax_breakdowns)}
	<table id="tax-tab" width="100%">
		...
		{if isset($order->payment_fee) && $order->payment_fee > 0 && $order->payment_fee_rate > 0}
			<tr>
				<td class="white">{l s='Administration fees' pdf='true'}</td>
				<td class="center white">{$order->payment_fee_rate} %</td>
				{if $display_tax_bases_in_breakdowns}
					<td class="right white">
					{if isset($is_order_slip) && $is_order_slip}- {/if}{displayPrice currency=$order->id_currency price=($order->payment_fee / (1+($order->payment_fee_rate/100)))}		
						</td>
					{/if}
				<td class="right white">{if isset($is_order_slip) && $is_order_slip}- {/if}{displayPrice currency=$order->id_currency price=($order->payment_fee * ($order->payment_fee_rate/100) / (1+($order->payment_fee_rate/100)))}</td>
			</tr>
		{/if}
